...
module.exports = class MetaLoader {
  ...
  async getLinkedObjectsModel(isTrigger = false) {
    let meta;
    if (isTrigger) {
      console.log("run");
      console.log("conf", this.configuration);
      console.log("confobj", this.configuration.object);
      meta = await this.connection.describe(this.configuration.object);
      console.log("meta", meta);
    } else {
      meta = await this.connection.describe(this.configuration.sobject);
    }
    const relatedObjs = {};
