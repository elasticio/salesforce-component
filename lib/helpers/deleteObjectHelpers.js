...
require('dotenv').config({path:__dirname+'../../spec-integration/actions/secrets.env'}); 
 ...
 * @implementedBy C.J.V
 * @returns true if all objects were sucessfully created
*/
module.exports.testDataFactory = async function* testDataFactory() {
  ...
  for ( let i = 0; i < deleteObjectData.cases.length; i++ ) {
      ...
      if ( JSON.stringify(this.emit.args[0][1]) === JSON.stringify({}) ) {
        yield false;
      }
