...
module.exports.deleteObjById = async function deleteObjById(sfConn, id, objType) {
  try {
    this.logger.trace(`Here's the ID of what we're deleting ${id}`);
    this.logger.trace(`Note if the object has a master-detail relationship 
                      with another object it will also be deleted `);
    const response = await sfConn.sobject(objType).delete(id);

    this.logger.trace(`Sucessfully deleted object ${id}`);
    const outputMessage = messages.newMessageWithBody({ response });
...
module.exports.testDataFactory = async function testDataFactory() {
  ...
  for (let i = 0; i < deleteObjectData.cases.length; i++) {
    const currCaseObj = deleteObjectData.cases[i];
    usesId = false;
    ...
    if (usesId) {
      currCaseObj.message.body.Id = this.emit.args[0][1].body.id;
    }

    // if there's a response then populate the ID dynamically
    if (currCaseObj.response) {
      currCaseObj.response.id = this.emit.args[0][1].body.id;
    }

    testObjLst.push(currCaseObj);
  }
  return testObjLst;
};
