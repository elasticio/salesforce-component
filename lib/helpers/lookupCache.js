// eslint-disable-next-line no-nested-ternary,max-classes-per-file
...
class LookupCache {
  constructor() {
    this.hashSize = process.env.HASH_LIMIT_ELEMENTS
      ? parseInt(process.env.HASH_LIMIT_ELEMENTS, 10) : 10;
  ...
  generateKeyFromDataArray(...params) {
    return params.reduce((a, b) => String(a) + String(b));
  }
