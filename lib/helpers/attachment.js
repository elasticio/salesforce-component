...
const { getCredentials } = require('../helpers/oauth2Helper');
...
exports.getAttachment = async function getAttachment(configuration, objectContent, emitter) {
  ...
  emitter.logger.trace('Fetched credentials: %j', credentials);
  const data = await downloadFile(credentials.instance_url + binDataUrl, {
