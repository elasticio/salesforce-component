...
function getObjectDescription(cfg, cb) {
    ...
    var metadataURL = util.format("%s/services/data/%s/sobjects/%s/describe", url, common.globalConsts.SALESFORCE_API_VERSION, objType);
...
function fetchObjectTypes(conf, cb) {
    ...
    var metadataURL = util.format("%s/services/data/%s/sobjects", url, common.globalConsts.SALESFORCE_API_VERSION);
