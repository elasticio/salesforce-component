...
module.exports.process = async function processAction(message, configuration) {
  ...
  lookupCache.useCache(this.configuration.enableCacheUsage);
  const queryKey = lookupCache.generateKeyFromDataArray(configuration.sobject, wherePart, offset, limit, configuration.includeDeleted);
