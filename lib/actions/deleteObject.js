  ...
  await conn.sobject(configuration.sobject)
    ...
    .on('end', async () => {
      if (res.length === 0) {
        ...
        try {
          this.logger.info(`Here's the result ${JSON.stringify(res)}`);
          this.logger.info(`Here's what we're deleting ${res[0].Id}`);
          const delConn = sfConnection.createConnection(configuration, this);
          let deleteRes = await delConn.sobject(configuration.sobject).delete(res[0].Id);
