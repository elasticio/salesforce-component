...
module.exports.getMetaModel = async function getMetaModel(configuration) {
  ...
  metaData.in.properties[configuration.lookupField].required = false;
  return metaData;
};
...
module.exports.process = async function process(message, configuration) {
  ...
  this.logger.debug(`Process is called with config: ${JSON.stringify(configuration)}`);
  this.logger.debug(`Process is called with message: ${JSON.stringify(message)}`);
