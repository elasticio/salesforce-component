...
function upsertObject(msg, cfg) {
  ...
  console.log('Upserting sobject=%s extIdField=%s data=%j', cfg.sobject,
      cfg.extIdField, msg);
  if (msg.body.Id) {
    return conn.sobject(cfg.sobject).update(msg.body).then(res=>{return getEntity(cfg, msg.body)});
  } else {
    return conn.sobject(cfg.sobject).upsert(msg.body, cfg.extIdField);
  }
}

function getEntity(cfg, body){
  const self = new SalesforceEntity(this);
  return self.processAction(cfg.sobject, body, cfg);
}
