...
exports.process = function processAction(msg, conf) {
  ...
  const data = conn.query(msg.body.query)
    ...
    .on('record', (record) => {
      if (allowResultAsSet) {
        ...
        result.push(record);
        this.emit('data', messages.newMessageWithBody(result[result.length-1]));
      }
    })
    .on('end', () => {
      ...
      this.logger.info('total in database=%s', data.totalSize);
      this.logger.info('total fetched=%s', data.totalFetched);
      this.emit('end');
    })
    .on('error', (err) => {
