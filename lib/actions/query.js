...
class EmitBatchHandler {
  ...
  async call() {
    ...
    const response = await new Promise((resolve, reject) => {
      this.connection.query(this.message.body.query)
        .scanAll(this.configuration.includeDeleted)
        .on('record', (record) => {
          batch.push(record);
          if (batch.length >= configuration.batchSize) {
            this.logger.info('Ready batch: %j', batch);
            promises.push(this.context.emit('data', messages.newMessageWithBody({ result: this.batch })));
            batch = [];
          }
        })
        .on('end', () => {
          ...
          if (response.totalFetched === 0) {
            promises.push(this.context.emit('data', messages.newMessageWithBody({})));
          }
          if (batch.length > 0) {
            promises.push(this.context.emit('data', messages.newMessageWithBody({ result: batch })));
          }
