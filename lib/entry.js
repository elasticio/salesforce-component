...
var debug = require('debug')('salesforce:entry');
...
exports.buildAction = function buildAction(objectType, exports) {
    ...
    exports.getMetaModel = function getActionMetaModel(cfg, cb) {
        ...
        self.getInMetaModel(cfg, function transformMetadata(err, data) {
            ...
            data.out.properties.externalID = {
                type: "string",
                title: "externalID"
            };
            cb(null, data);
    ...
    exports.externalIdField = function externalIdField(cfg, cb) {
        ...
        // TODO will emit refresh work here?
        // conn.on("refresh", function refreshToken(accessToken, res) {
        //     debug('Keys were updated, res=%j', res);
        //     self.emit('updateKeys', { oauth : res });
        // });
        conn.sobject(objectType).describe(function parseDescription(err, meta) {
            if (err) {
                return cb(err);
            }
            var result = {};
            meta.fields.filter(function filterField(field) {
                return !field.calculated;
            }).map(function createResult(field) {
