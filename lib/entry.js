...
function SalesforceEntity(callScope) {
  ...
  this.processTrigger = async function processTrigger(msg, cfg, snapshot) {
    ...
    if (snapshot || typeof snapshot !== 'string' || snapshot === '') {
      // Empty snapshot
      console.warn('Warning: expected string in snapshot but found : ',
          snapshot);
      snapshot = new Date('1970-01-01T00:00:00.000Z').toISOString();
    } else {
    ...
    await conn.sobject(cfg.object)
        .select('*')
        .where(`LastModifiedDate > ${snapshot}`)
        .execute({ autoFetch : true, maxFetch : 10000 }, processResults);

    function processResults(err, records) {
      if (err) {
        onError(err);
      }
      if (!records.length) {
      ...
      console.log('emitting snapshot: %j', newSnapshot);
  ...
  this.processAction = function processAction(objectType, msg, conf) {
    ...
    function postData(conf, next) {
      ...
      var url = util.format('%s/services/data/%s/sobjects/%s', baseUrl, version,
          objectType);
