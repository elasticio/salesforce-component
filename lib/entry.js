...
function SalesforceEntity(callScope) {
  ...
  function getMetaModel(direction, cfg, cb) {
    ...
    self.refreshToken(cfg, (err, newCfg) => {
      if (err) {
        callScope.logger.error('Error refreshing token');
        return cb(err);
      }
  ...
  this.processTrigger = async function processTrigger(msg, cfg, snapshot) {
    ...
    conn.on('refresh', (accessToken, res) => {
      callScope.logger.debug('Keys were updated');
      emitKeys({ oauth: res });
    });

    let maxFetch = cfg.maxFetch ? cfg.parseInt(cfg.maxFetch, 10) : 1000;

    if (cfg.sizeOfPollingPage) {
      callScope.logger.debug('Current sizeOfPollingPage=%s, maxFetch=%s', cfg.sizeOfPollingPage, maxFetch);
    ...
    let query = conn.sobject(cfg.object)
    ...
    if (snapshot.previousLastModified !== lastSeenTime) {
      ...
      callScope.logger.debug('emitting new snapshot');
  ...
  this.listLinkedObjects = function listLinkedObjects(conf, next) {
    // eslint-disable-next-line consistent-return
    self.refreshToken(_.clone(conf), async (err, newCfg) => {
      if (err) {
        return next(err);
      }
      try {
        newCfg.sobject = newCfg.object;
        const metaLoader = new MetaLoader(newCfg, this);
        const relatedObjs = await metaLoader.getLinkedObjectsModel();
  ...
  this.processAction = function processAction(objectType, msg, conf) {
    ...
    function postData(config, next) {
      ...
      callScope.logger.debug('getJSON');
  ...
  function emitError(err) {
    callScope.logger.error('emitting SalesforceEntity error');
