...
function SalesforceEntity(callScope) {
  ...
  this.processTrigger = async function processTrigger(msg, cfg, snapshot) {
    ...
    if (!snapshot || (typeof(snapshot) === 'object' && !snapshot.previousLastModified)) {
      snapshot = {
        previousLastModified: cfg.startTime || new Date('1970-01-01T00:00:00.000Z').toISOString(),
      };
    ...
    const maxFetch = cfg.sizeOfPollingPage || 1000;
    ...
    do {
      let whereCondition;
      if (lastSeenTime === cfg.startTime || lastSeenTime === new Date('1970-01-01T00:00:00.000Z').toISOString())
      ...
      await conn.sobject(cfg.object)
        ...
        .execute({ autoFetch: true, maxFetch }, processResults);
