...
function SalesforceEntity(callScope) {
  ...
  this.processTrigger = async function processTrigger(msg, cfg, snapshot) {
    ...
    if (!snapshot || (typeof(snapshot) === 'object' && !snapshot.previousLastModified)) {
      ...
      logger.info('Created snapshot: %j', snapshot)
    } else if (typeof snapshot === 'string') {
      // for backward compatibility
      snapshot = { previousLastModified: snapshot };
      logger.info('Snapshot has been converted to: %j', snapshot);
    } else {
      logger.info('Got snapshot: %j', snapshot)
    }
    ...
    conn.on('refresh', (accessToken, res) => {
      logger.info('Keys were updated, res=%j', res);
      emitKeys({ oauth: res });
    });
    ...
    if (snapshot.previousLastModified !== lastSeenTime) {
        ...
        logger.info('emitting new snapshot: %j', snapshot);
    ...
    function processResults(records) {
      ...
      records.forEach((record, i) => {
        ...
        msg.body = record;
        logger.info('emitting record %d', i);
        logger.debug('emitting record: %j', msg);
        emitData(msg);
      });
  ...
  function emitError(err) {
    logger.error('emitting SalesforceEntity error', err, err.stack);
