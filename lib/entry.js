...
function SalesforceEntity(callScope) {
   ...
   * @param snapshot
   */
  this.processTrigger = async function processTrigger(msg, cfg, snapshot, version = SALESFORCE_API_VERSION) {
  ...
  this.processQuery = function processQuery(query, cfg) {
    ...
    const params = {};
    params.cfg = cfg;
    params.cfg.apiVersion = `v${SALESFORCE_API_VERSION}`;
   ...
   * @param conf
   */
  this.processAction = function processAction(objectType, msg, conf, version = SALESFORCE_API_VERSION) {

    self.refreshToken(conf, function onTokenRefreshed(err, newCfg) {
      if (err) {
        return onActionError(err);
      }
      postData(newCfg, checkPostResult, version);
    });

    function postData(conf, next, version = SALESFORCE_API_VERSION) {
      ...
      const url = util.format('%s/services/data/%s/sobjects/%s', baseUrl, `v${version}`,
          objectType);
...
exports.buildAction = function buildAction(objectType, exports, version = SALESFORCE_API_VERSION) {

  exports.process = function processAction(msg, conf) {
    const self = new SalesforceEntity(this);
    self.processAction(objectType, msg, conf, version);
  };
...
exports.buildTrigger = function buildTrigger(objectType, exports, version=SALESFORCE_API_VERSION) {
  ...
  exports.process = function processTrigger(msg, conf, snapshot) {
    ...
    // Proceed as usual
    self.processTrigger(msg, conf, snapshot, version);
  };
