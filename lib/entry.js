...
module.exports.process = async function process(message, configuration, snapshot) {
  if (!snapshot || (typeof (snapshot) === 'object' && !snapshot.previousLastModified)) {
    ...
    this.logger.info('Created snapshot: %j', snapshot);
  } else if (typeof snapshot === 'string') {
    // for backward compatibility
    snapshot = { previousLastModified: snapshot };
    this.logger.info('Snapshot has been converted to: %j', snapshot);
  } else {
    this.logger.info('Got snapshot: %j', snapshot);
  }
  ...
  if (configuration.sizeOfPollingPage) {
    this.logger.info('Current sizeOfPollingPage=%s, maxFetch=%s', configuration.sizeOfPollingPage, maxFetch);
      ...
      if (!records || !records.length) {
        ...
        if (outputMethod === 'emitAll') {
          ...
          for (let i = 0; i < records.length; i += 1) {
            ...
            this.logger.info('emitting record %d', i);
  ...
  if (snapshot.previousLastModified !== lastSeenTime) {
    ...
    this.logger.debug('emitting new snapshot: %j', snapshot);
