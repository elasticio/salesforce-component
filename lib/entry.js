...
function SalesforceEntity(callScope) {
  ...
  this.listLinkedObjects = function listLinkedObjects(conf, next) {
    const metaLoader = new MetaLoader(conf, this);
    // eslint-disable-next-line consistent-return
    self.refreshToken(_.clone(conf), (err) => {
      if (err) {
        return next(err);
      }
      metaLoader.getLinkedObjectsModel();
    });
  };
...
exports.linkedObjectTypes = function linkedObjectTypes(conf) {
  const metaLoader = new MetaLoader(conf, this);
  metaLoader.getLinkedObjectsModel();
};
