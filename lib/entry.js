              ...
              ...objectDescription.fields.filter(field => field.type === 'reference')
                .reduce((obj, field) => {
                  if (!field.referenceTo.length) {
                    throw new Error(
                      `Empty referenceTo array for field of type 'reference'
                       with name ${field.name} field=${JSON.stringify(
                        field, null, ' ',
                      )}`,
                    );
                  }
              ...
              ...objectDescription.childRelationships
                .reduce((obj, child) => {
                  if (child.relationshipName) {
                    ...
                    // eslint-disable-next-line no-param-reassign
                    obj[`!${child.relationshipName}`] =
                      `${child.childSObject} (${child.relationshipName})`;
                  }
                  return obj;
                }, {}),
            };
