...
function SalesforceEntity(callScope) {
  ...
  this.processTrigger = async function processTrigger(msg, cfg, snapshot) {
    ...
    function processResults(records) {
      ...
      if (cfg.outputMethod === 'emitAll') {
        emitData({ records });
      } else if (cfg.outputMethod === 'emitIndividually') {
  ...
  this.processQuery = function processQuery(query, cfg) {
    ...
    function processResults(results) {
      ...
      if (params.cfg.outputMethod === 'emitIndividually') {
        records.forEach(emitResultObject);
      } else if (params.cfg.outputMethod === 'emitAll') {
...
exports.processQuery = function processQuery(msg, conf, snapshot) {
  const self = new SalesforceEntity(this);
  self.processQuery(msg, conf, snapshot);
};
