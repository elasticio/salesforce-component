...
function SalesforceEntity(callScope) {
  ...
  this.processTrigger = async function processTrigger(msg, cfg, snapshot) {
    ...
    do {
      let whereCondition;
      if (lastSeenTime === cfg.startTime || lastSeenTime === '1970-01-01T00:00:00.000Z') { whereCondition = `LastModifiedDate >= ${lastSeenTime}${maxTime}`; } else { whereCondition = `LastModifiedDate > ${lastSeenTime}${maxTime}`; }
      try {
  ...
  this.processAction = function processAction(objectType, msg, conf) {
    self.refreshToken(conf, (err, newCfg) => {
      ...
      postData(newCfg, checkPostResult, SALESFORCE_API_VERSION);
    });
    ...
    function postData(conf, next) {
      ...
      const url = util.format('%s/services/data/%s/sobjects/%s', baseUrl, `v${SALESFORCE_API_VERSION}`,
        objectType);
...
exports.buildAction = function buildAction(objectType, exports) {
  exports.process = function processAction(msg, conf) {
    ...
    self.processAction(objectType, msg, conf, SALESFORCE_API_VERSION);
  };
